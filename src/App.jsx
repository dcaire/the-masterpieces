...
/* ---------- shared singer-availability list ---------- */
function AvailabilityRows({ aR, ea, onSet }) {
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>{aR.map(m => { const pcv = VP[m.voice_part] || VP.Soprano; const resp = ea[m.id] || 'pending'; return <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 13px', background: '#faf5e9', borderRadius: 11 }}>
    ...
    <div style={{ display: 'flex', gap: 5 }}>{['yes', 'pending', 'no'].map(r => { const c = RESP[r]; const on = resp === r; return <button key={r} onClick={() => onSet(m.id, r)} style={{ padding: '6px 12px', borderRadius: 9, fontSize: 11.5, fontWeight: 700, textTransform: 'capitalize', background: on ? c.fg : '#eee', color: on ? '#fff' : '#9ca3af' }}>{r === 'pending' ? 'maybe' : r}</button> })}</div>
  </div> })}</div>
}
